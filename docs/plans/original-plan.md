...
apl-service/
  ...
  APLSource/
    ...
    CheckFolder.aplf         # folder -> 'ok' | 'not-found' | 'not-a-directory'
...
- **`CheckFolder folder`** - `⎕NEXISTS` then `1 ⎕NINFO` (type); returns `'ok'`, `'not-found'`, or `'not-a-directory'`. Readability is not a distinct outcome: an unreadable folder surfaces as a mid-scan error that `Get`'s `:Trap 0` turns into a 500. Takes whichever absolute folder path it's given, root or a resolved subfolder; it has no notion of "the" root.
...
4. Manually confirm the flagged open question before relying on it in tests: whether `⎕NINFO` mtimes are local or UTC.
